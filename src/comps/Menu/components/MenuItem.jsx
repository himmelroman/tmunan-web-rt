...
export const MenuItem = withHovering(
	...
	function MenuItem({
		...
		const kids = useMemo(() => safeCall(children, modifiers), [children, modifiers])

		return isAnchor ? (
			<li role={roleNone}>
				<a href={href} {...menuItemProps} data-text>
					{kids}
				</a>
			...
			<li {...menuItemProps}>
				<span data-text>{kids}</span>
				{hotkey && <span className='hotkey'>{hotkey.split('+').join(' + ')}</span>}
			</li>
		)
	}
)
